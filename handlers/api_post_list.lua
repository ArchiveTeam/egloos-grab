...
module.get_urls = function(file, url, is_css, iri)
    ...
    if json["post"] ~= nil then
        local domain, page = current_options.url:match("^https://api%.egloos%.com/(.+)/post%.json%?page=(%d+)$")
        ...
        queue_request({url="https://api.egloos.com/" .. domain .. "/post.json?page=" .. tostring(tonumber(page) + 1)}, "api_post_list", false)
    end
end
